...
const addSchool = async () => {
  ...
  try {
    ...
    if (!existingClass) {
      const resClass = await fetch(`${API_URL}/classes`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    const resTeacher = await fetch(`${API_URL}/teachers`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
...
const deleteTeacher = async (id: string) => {
  ...
  try {
    ...
    const currentClass = state.classes.find(
      c => c.id === state.currentUser?.classId
    );
...
const updateStudent = async () => {
  ...
  try {
    const res = await fetch(`${API_URL}/students/${editingItem.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
...
const submitAllStudents = async () => {
  try {
    ...
    const draftStudents = state.students.filter(s =>
      s.classId === state.currentUser?.classId &&
      s.status === "draft"
    );
    
    console.log("Found draft students:", draftStudents.length);
    
    if (draftStudents.length === 0) {
      alert("No draft students to submit");
      return;
    }

    const updated = await Promise.all(
      draftStudents.map(s =>
        fetch(`${API_URL}/students/${s.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            status: "submitted"
    ...
    console.log("API calls completed:", updated.length);

    // Check for errors
    const failedUpdates = updated.filter(res => !res.ok);
    if (failedUpdates.length > 0) {
      console.error("Some updates failed:", failedUpdates);
      alert(`Failed to submit ${failedUpdates.length} students`);
      return;
    }
    ...
    console.log("Refetched students:", students.length);
    ...
    alert(`Successfully submitted ${draftStudents.length} students!`);

  } catch (error) {
    console.error("Submit error:", error);
    alert("Error submitting students. Please try again.");
  }
};
          ...
          <div>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-1">View all student data across all schools and classes</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                {/* 👇 ADD THIS DROPDOWN */}
                <select
            ...
            {/* Admin can see all students - grouped by school and class */}
            {(() => {
